...
extern float xsdot_(int*, float*, int*, float*, int*);

float f_sdot(int n, float *x, int incx, float*y, int incy)
{
  return xsdot_(&n,x,&incx,y,&incy);
}
...
{ 
  ...
  float ans = sdot(N,x,incx,y,incy);
...
{
  clock_gettime(CLOCK_MONOTONIC, &begin);
  float ans = f_sdot(N,x,incx,y,incy);
  clock_gettime(CLOCK_MONOTONIC, &end);
  unsigned long long int time = 1000000000L*(end.tv_sec - begin.tv_sec) + end.tv_nsec - begin.tv_nsec;
  printf("%16f%16lld",ans,time);
  return time;
...
int main(int argc, char** argv) 
{
  ...
  printf("Running test for SDOT with N=%d\n",N);
  printf("%32s%32s\n","ACML","Fortran");
  printf("Trials\n");
