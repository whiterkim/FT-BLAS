...
extern double xdsdot_(int*, float*, int*, float*, int*);

double f_dsdot(int n, float *x, int incx, float *y, int incy)
{
  return xdsdot_(&n,x,&incx,y,&incy);
}
...
unsigned long long int acml_time()
{
  ...
  double ans = dsdot(N,x,incx,y,incy);
...
{
  clock_gettime(CLOCK_MONOTONIC, &begin);
  double ans = f_dsdot(N,x,incx,y,incy);
  clock_gettime(CLOCK_MONOTONIC, &end);
  unsigned long long int time = 1000000000L*(end.tv_sec - begin.tv_sec) + end.tv_nsec - begin.tv_nsec;
  printf("%16f%16lld",ans,time);
  return time;
...
int main(int argc, char** argv) 
{
  ...
  printf("Running test for DSDOT with N=%d\n",N);
  printf("%32s%32s\n","ACML","Fortran");
  printf("Trials\n");
