...
extern void	xcswap_(int *n, complex *x, int *incx, complex *y, int *incy);
...
int N, incx, incy;
complex *x_1, *y_1, *x_2, *y_2,*t_x, *t_y;

struct timespec begin, end;
unsigned long long int acml_time()
{
  memcpy(x_1,t_x,sizeof(complex)*N);
  memcpy(y_1,t_y,sizeof(complex)*N);

  clock_gettime(CLOCK_MONOTONIC,&begin);
  cswap(N,x_1,incx,y_1,incy);
...
{
  memcpy(x_2,t_x,sizeof(complex)*N);
  memcpy(y_2,t_y,sizeof(complex)*N);

  clock_gettime(CLOCK_MONOTONIC,&begin);
  f_cswap(N,x_2,incx,y_2,incy);
  clock_gettime(CLOCK_MONOTONIC,&end);
...
int array_cmp()
{
  int n1 = memcmp(x_1,x_2,sizeof(complex)*N);
  int n2 = memcmp(y_1,y_2,sizeof(complex)*N);
...
void run_test() 
{
  printf("Running test for CSWAP with N = %d, incx = %d, incy = %d\n", N, incx, incy);
  printf("%32s%32s\n","ACML", "Fortran");
  printf("Trials\n");
...
int main(int argc, char *argv[])
{
  ...
  x_2 = (complex*)malloc(sizeof(complex)*N);
  ...
  x_2 = (complex*)malloc(sizeof(complex)*2*N);
