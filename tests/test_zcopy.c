...
extern void	xzcopy_(int *n, doublecomplex *x, int *incx, doublecomplex *y, int *incy);

void f_zcopy(int n, doublecomplex *x, int incx, doublecomplex *y, int incy)
{
  xzcopy_(&n,x,&incx,y,&incy);
}       
...
int array_cmp()
{
  int n1 = memcmp(y_1,y_2,sizeof(doublecomplex)*N);
...
void run_test() 
{
  printf("Running test for ZCOPY with N = %d, incx = %d, incy = %d\n", N, incx, incy);
  printf("%32s%32s\n","ACML", "Fortran");
  printf("Trials\n");
...
int main(int argc, char *argv[])
{
  ...
  y_1 = (doublecomplex*)malloc(sizeof(doublecomplex)*2*N);
  y_2 = (doublecomplex*)malloc(sizeof(doublecomplex)*2*N);
  run_test();
