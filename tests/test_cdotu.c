...
extern complex xcdotu_(int*, complex*, int*, complex*, int*);

complex f_cdotu(int n, complex *x, int incx, complex *y, int incy)
{
  return xcdotu_(&n,x,&incx,y,&incy);
}
...
int scalar_cmp()
{
  if (fabs(ans1.real - ans2.real) > seps)
    return 0;
  else if (fabs(ans1.imag - ans2.imag) > seps)
    return 0;
  else 
    return 1;
}
...
void run_test() 
{
  printf("Running test for CDOTU with N = %d, incx = %d\n", N, incx);
  printf("%32s%32s\n","ACML","Fortran");
  printf("Trials\n");
