...
extern int xizamax_(int *n, doublecomplex *x, int *incx);

int f_izamax(int n, doublecomplex *x, int incx)
{
  return xizamax_(&n,x,&incx);
}
...
void run_test() 
{
  printf("Running test for IZAMAX with N = %d, incx = %d\n", N, incx);
  printf("%32s%32s\n","ACML","Fortran");
  printf("Trials\n");
