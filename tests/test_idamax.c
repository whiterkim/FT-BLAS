...
extern int xidamax_(int *n, double *x, int *incx);

int f_idamax(int n, double *x, int incx)
{
  return xidamax_(&n,x,&incx);
}
...
void run_test() 
{
  printf("Running test for IDAMAX with N = %d, incx = %d\n", N, incx);
  printf("%32s%32s\n","ACML","Fortran");
  printf("Trials\n");
