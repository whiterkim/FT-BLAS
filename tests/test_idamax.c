...
extern int xidamax_(int*, double*, int*);

int c_idamax(int n, double *x, int incx)
{
  register int imax = 0;
  register double max = fabs(x[0]);
  register int i;
  for (i = 1; i < n; ++i) {
    if (max < fabs(x[i])) {
      imax = i;
...
int main(int argc, char** argv) 
{
  ...
  printf("Running test for IDAMAX with N=%d\n",N);
  printf("%32s%32s%32s\n","ACML","C","Fortran");
  printf("Trials\n");
