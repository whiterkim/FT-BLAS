...
extern int xisamax_(int *n, float *x, int *incx);

int f_isamax(int n, float *x, int incx)
{
  return xisamax_(&n,x,&incx);
}
...
void run_test() 
{
  printf("Running test for ISAMAX with N = %d, incx = %d\n", N, incx);
  printf("%32s%32s\n","ACML","Fortran");
  printf("Trials\n");
