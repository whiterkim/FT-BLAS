...
extern	void	xzdscal_(	const int*n,	const double*alpha,	doublecomplex *f_x,			const int*incx);

// Fortran 
void f_zdscal(int n, double alpha, doublecomplex *x, int incx)
{
  xzdscal_(&n,&alpha,x,&incx);
}
...
int array_cmp()
{
  ...
  for(i = 0; i < N; ++i)
  {
    if(fabs(x[i].real - x_1[i].real) > deps)
      return 0;
    else if(fabs(x[i].imag - x_1[i].imag) > deps)
      return 0;
  }
  return 1;
}
...
void run_test()
{
  ...
  printf("Running test for ZDSCAL with N = %d, incx = %d, alpha = %f\n", N, incx, alpha);
  printf("%32s%32s\n","ACML", "FORTRAN");
  printf("Trials\n");
