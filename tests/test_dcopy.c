...
extern void	xdcopy_(int *n, double *x, int *incx, double *y, int *incy);

void f_dcopy(int n, double *x, int incx, double *y, int incy)
{
  xdcopy_(&n,x,&incx,y,&incy);
}       
...
int array_cmp()
{
  int n1 = memcmp(y_1,y_2,sizeof(double)*N);
...
void run_test() 
{
  printf("Running test for DCOPY with N = %d, incx = %d, incy = %d\n", N, incx, incy);
  printf("%32s%32s\n","ACML", "Fortran");
  printf("Trials\n");
