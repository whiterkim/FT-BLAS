...
extern void	xsswap_(int *n, float *x, int *incx, float *y, int *incy);

void f_sswap(int n, float *x, int incx, float *y, int incy)
{
  xsswap_(&n,x,&incx,y,&incy);
}       

int N, incx, incy;
float *x_1, *y_1, *x_2, *y_2,*t_x, *t_y;
...
{
  memcpy(x_2,t_x,sizeof(float)*N);
  memcpy(y_2,t_y,sizeof(float)*N);

  clock_gettime(CLOCK_MONOTONIC,&begin);
  f_sswap(N,x_2,incx,y_2,incy);
  clock_gettime(CLOCK_MONOTONIC,&end);
...
int array_cmp()
{
  int n1 = memcmp(x_1,x_2,sizeof(float)*N);
  int n2 = memcmp(y_1,y_2,sizeof(float)*N);
...
void run_test() 
{
  printf("Running test for SSWAP with N = %d, incx = %d, incy = %d\n", N, incx, incy);
  printf("%32s%32s\n","ACML", "Fortran");
  printf("Trials\n");
...
int main(int argc, char *argv[])
{
  ...
  x_2 = (float*)malloc(sizeof(float)*N);
