...
extern float xsasum_(int*, float*, int*);

float c_sasum(int n, float *x, int incx)
{
  register float asum = 0;
  register int i;
  for (i = 0; i < n; ++i) {
    asum += fabs(x[i]);
  }
...
int main(int argc, char** argv) 
{
  ...
  printf("Running test for SASUM with N=%d\n",N);
  printf("%32s%32s%32s\n","ACML","C","Fortran");
  printf("Trials\n");
