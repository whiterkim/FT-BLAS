...
300         xi = x(ix)
     ...
     $         (abs(aimag(x1) - aimag(x2)).ge. seps)) goto 300
      ...
      subroutine xzscal(n,alpha,x,incx)

      integer incx,n
      complex*16 x(*),alpha
      complex*16 x11, x12, x21, x22, x31, x32, x41, x42, x51, x52
      complex*16 xi, x1, x2
      integer i,ix, m, mp1
...
300         x1 = x(ix) * alpha
     ...
     $         (abs(aimag(x1) - aimag(x2)).ge. seps)) goto 300
      ...
      subroutine xzdscal(n,alpha,x,incx)

      integer incx,n
      complex*16 x(*)
      double precision alpha
      
      complex*16  x1, x2, x11, x12, x21, x22, x31, x32, x41, x42,
     $         x51, x52, xi
      integer i,ix
      ...
      if (incx.eq.1) then
        ...
        if(m.NE.0) then
            do i = 1,m
100             xi = x(i)
                x1 = dcmplx(dble(xi) * alpha, aimag(xi) * alpha)
                x2 = dcmplx(dble(xi) * alpha, aimag(xi) * alpha)

                if((abs(dble(x1) - dble(x2)).ge. deps).or.
     $             (abs(aimag(x1) - aimag(x2)).ge. deps)) goto 100
                x(i) = x1
            end do
        end if
        if (n.lt.5) return
        mp1 = m + 1
        do i = mp1, n, 5
200         x11 = dcmplx(dble(x(i)) * alpha, aimag(x(i)) * alpha)
            x21 = dcmplx(dble(x(i+1)) * alpha, aimag(x(i+1)) * alpha)
            x31 = dcmplx(dble(x(i+2)) * alpha, aimag(x(i+2)) * alpha)
            x41 = dcmplx(dble(x(i+3)) * alpha, aimag(x(i+3)) * alpha)
            x51 = dcmplx(dble(x(i+4)) * alpha, aimag(x(i+4)) * alpha)

            x12 = dcmplx(dble(x(i)) * alpha, aimag(x(i)) * alpha)
            x22 = dcmplx(dble(x(i+1)) * alpha, aimag(x(i+1)) * alpha)
            x32 = dcmplx(dble(x(i+2)) * alpha, aimag(x(i+2)) * alpha)
            x42 = dcmplx(dble(x(i+3)) * alpha, aimag(x(i+3)) * alpha)
            x52 = dcmplx(dble(x(i+4)) * alpha, aimag(x(i+4)) * alpha)
            
            x1 = x11 + x21 + x31 + x41 + x51
            x2 = x12 + x22 + x32 + x42 + x52

            if((abs(dble(x1) - dble(x2)).ge. deps).or.
     $         (abs(aimag(x1) - aimag(x2)).ge. deps)) goto 200
            x(i) = x11
            x(i+1) = x21
            x(i+2) = x31
            x(i+3) = x41
...
300         xi = x(ix)
            ...
            if((abs(dble(x1) - dble(x2)).ge. deps).or.
     $         (abs(aimag(x1) - aimag(x2)).ge. deps)) goto 300
            x(ix) = x1
